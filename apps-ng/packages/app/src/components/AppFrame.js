...
const AppFrameWrapper = styled.div`
  padding: calc(${constants.NAV_HEIGHT}px + ${constants.CONTAINER_PADDING}px) 0
    ${constants.CONTAINER_PADDING}px;
`
...
const NavBarWrapper = styled.nav`
  ...
  box-shadow: 0 1px 0 rgba(255, 255, 255, 0.21);
...
export const NavBarButtonWrapper = styled.div`
  ...
  box-shadow: 0 0 0 1.5px
    ${(props) => (props.active ? "#FFFFFF" : "transparent")} inset;
  box-sizing: border-box;
  border-radius: 6px;
  flex-direction: row;
  align-items: flex-end;
...
const NavBar = () => {
  ...
  return (
    <NavBarWrapper>
      <Container>
        <PhalaLogo
          onClick={() => window.open("https://phala.network/", "_blank")}
        />
        <NavLogoSpacer />
        ...
        <NavItemSpacer />
        <NavBarButton href="GPSAPP" name={t("Gps App")} icon={GlobeIcon} />
        <NavItemSpacer />
        <NavBarButtonWrapper
...
const NavBarButton = ({ href, icon, name }) => {
  ...
  return (
    <RouteLink href={href}>
      <NavBarButtonWrapper
        active={MENU_ROUTES[href] === `/${router?.query.slug?.[0]}`}
      >
        <Icon size={21} />
  ...
  return (
    <AppFrameWrapper>
      ...
      <_Breadcrumbs separator="" size="small">
        ...
        <Breadcrumbs.Item href="https://forum.phala.network/" target="_blank">
          Forum
        </Breadcrumbs.Item>
        <Breadcrumbs.Item href="https://t.me/phalanetwork" target="_blank">
          Telegram
        </Breadcrumbs.Item>
        <Breadcrumbs.Item href="https://discord.gg/FUtZzYH" target="_blank">
          Discord
        </Breadcrumbs.Item>
        <Breadcrumbs.Item
