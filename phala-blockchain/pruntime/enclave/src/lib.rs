...
use contracts::{AccountIdWrapper, Contract, ContractId, DATA_PLAZA, BALANCE, ASSETS, SYSTEM, WEB3_ANALYTICS, HELLO_WORLD,GPS_APP};
...
struct RuntimeState {
    ...
    contract6: contracts::gps_location::GpsLocation,
...
lazy_static! {
    static ref STATE: SgxMutex<RuntimeState> = {
        SgxMutex::new(RuntimeState {
            ...
            contract6: contracts::gps_location::GpsLocation::new(),
...
fn handle_execution(state: &mut RuntimeState, pos: &TxRef,
    ...
    println!("handle_execution: about to call handle_command {}",contract_id);
    ...
    let status = match contract_id {
        ...
        GPS_APP => {
            let slice =serde_json::from_slice(inner_data.as_slice());
            println!("GPS APP SLICE {:?}",slice);
            
            if let Ok(cmd) = slice{
                println!("Handle command {:?} {:?} {:?}",origin,pos,cmd);
                state.contract6.handle_command(&origin, pos, cmd)
            }else{
                TransactionStatus::BadCommand
            }
...
fn query(q: types::SignedQuery) -> Result<Value, Value> {
    ...
    let res = match opaque_query.contract_id {
        ...
        GPS_APP => {
            let request =  types::deopaque_query(opaque_query)
            .map_err(|_| error_msg("Malformed request (gpsApp::Request)"))?.request;

            println!("GPS APP QUERY HIT");
            println!("Opaque Query {:?}",&request);

            let contract_resp =   state.contract6.handle_query( accid_origin.as_ref(), request);

            let json = serde_json::to_value(contract_resp).unwrap();



            println!("GPS APP REPLY Query {:?}",json);
